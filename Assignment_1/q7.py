...
def all_agree(S, G, x):
    """
     determines whether all the hypotheses in the version space agree with the classification of x (i.e. have the same prediction).
     通过所有的S 和 G 得到相同的结果 是否一致
    """
    H = S | G  # all hypotheses

    result = []
    for h in H:
        # h == "?" or h == x   ## ? means any
        result.append(all(x[i] == h[i] or h[i] == "?" for i in range(len(x))))

    # sum []  True is one False is 0
    return sum(result) == 0 or sum(result) == len(result)


# def all_agree(S, G, x):
#     sg = G | S
#     ans = [True for none in range(len(sg))]
#     for i, j in enumerate(sg):
#         for a in range(len(x)):
#             if j[a] != x[a] and j[a] != '?':
#                 ans[i] = False
#     return ans.count(False) == len(sg) or ans.count(True) == len(sg)
